...
class ProfileController  extends AppController {
	...
	function editAlbum(){
		...
		if($id!=null){
			...
			if($this->Member->Album->updateAll(
				array('Album.title' => "'$title'" ),
				array('Album.id' => $id, 'Album.member_id' => $this->Cookie->read('Member.id')
			))) 
				return $title;
		}else
			return false;
	...
	function confirm($memberId = null, $code = null) {
		...
		if ($member['Member']['confirm_code'] == $code) {
			...
			$this -> Session -> setFlash(__('Registration confirmed. You can now <a class="inline" href="#loginForm">Log In</a>.', true));
		} else {
	...
	function login() {
		...
		if (!empty($this -> data)) {
			...
			$member = $this -> Member -> find('first', array(
				'fields' => array ('id', 'name', 'email', 'password', 'phone', 'country', 'city', 'area', 'address', 'confirmed'),
