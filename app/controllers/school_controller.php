...
class SchoolController extends AuthController {
    ...
    function branches($parentId) {
        $this->Member->recursive = 1;

        $this->paginate = array(
            'conditions' => $this->arrKeyPrefix(array_merge(array('parent_id' => $parentId, 'type' => 1), (array) $this->Session->read('conditions')), 'Member'),
            'limit' => isset($this->params['named']['limit']) ? $this->params['named']['limit'] : $this->paginate['limit'],
            'page' => isset($this->params['named']['page']) ? $this->params['named']['page'] : $this->paginate['page'],
        );
        $parent = $this->Member->find('all', array(
            'conditions' => array(
                'Member.id' => $parentId
            )
        ));
        $parentTitle = $parent[0]['Member']['name'];
    ...
    function add($id = null) {
        ...
        if (!empty($this->data)) {
            ...
            if ($this->Member->save($this->data, array('deep' => true))) {
...
//                array_unshift($memberList, "Choose Parent");
        ...
        if ($id) {
            ...
            $parentName = $parent[0]['Member']['name'];
        } else {
            $this->set('mainId', 0);
        }
    ...
    function saveGrades() {
        foreach ($this->data['Friend'] as $friend) {
            //set article data
            $friend['member_id'] = $this->Member->id;

            $friend['title'] = $friend['title'];
            $friend['friend_id'] = $friend['friend_id'];
            $friend['parent_id'] = $friend['parent_id'];
            //Save album
            $this->Member->Friend->create();

            $this->Member->Friend->save($friend);
        }
    }
